	...
	if(!empty($_POST['email'])){
		...
		if($prevUser > 0){
			...
			if($update){
				...
				//YOU'LL HAVE TO CHANGE THIS TO THE RIGHT LOCATION!!

				$resetPassLink = 'http://old-friends.co/resetPassword.php?fp_code='.$uniqidStr;
				...
				//additional headers
				$headers .= 'From: old-friends.co<admin@old-friends.co>' . "\r\n";
				//send email
	...
	if(!empty($_POST['password']) && !empty($_POST['confirm_password']) && !empty($_POST['fp_code'])){
		...
		if($_POST['password'] !== $_POST['confirm_password']){
			...
			if(!empty($prevUser)){
				...
				$sessData['status']['type'] = 'error';
				$sessData['status']['msg'] = 'You are not authorized to reset the password of this account.';
			}
		}
	}else{
		$sessData['status']['type'] = 'error';
